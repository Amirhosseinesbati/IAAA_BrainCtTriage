#!/usr/bin/env bash
# Strict one-checkpoint CUDA resource screen for pre-registered MLS A4.
set -Eeuo pipefail
...
audit_artifact_root="$campaign_root/a4_fold0_seed42_cuda_audit"
...
decision="$report_root/A4_FOLD0_SEED42_RESOURCE_SCREEN_DECISION.json"
...
if [[ -f "$primary_training_report" && -f "$canonical_training_report" ]]; then
  echo "A4 training report is ambiguous across source roots" >&2
...
if [[ ! "$mlflow_run_id" =~ ^[0-9a-f]{32}$ ]]; then
  echo "Could not recover a valid MLflow run id from the completed A4 report" >&2
...
set +e
"$project_root/.venv/bin/python" scripts/evaluate_mls_a4_fold0_resource_screen.py \
...
printf 'A4 fold-0 seed-42 resource screen completed: %s\n' "$decision"
