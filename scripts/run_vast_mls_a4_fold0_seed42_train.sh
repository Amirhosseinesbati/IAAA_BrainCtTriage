#!/usr/bin/env bash
# One-GPU, checksum-bound launcher for the preregistered MLS A4 resource screen.
set -Eeuo pipefail
...
run_name="mls-vast-da-a4-pair-rank-fold0-seed42"
manifest="$project_root/config/experiments/mls-vast-deploy-aligned-a4-pair-rank-template.yaml"
...
source_commit_local="6ddd738244cc8b5d702235e64c88b1c8608a93f3"
...
write_status() {
  ...
  printf '{"schema_version":1,"state":"%s","exit_code":%s,"run_name":"%s","manifest":"%s","manifest_sha256":"%s","source_commit_local":"%s","log_path":"%s","started_utc":"%s","finished_utc":"%s","compute_policy":"cuda_only_no_cpu_fallback","auto_destroy":false}\n' \
    "$state" "$exit_code" "$run_name" "$manifest_snapshot" "${manifest_sha256:-}" \
    "$source_commit_local" "$log_path" "${started_utc:-}" "$timestamp" \
    >"$status_path.tmp"
...
if [[ ! -f "$manifest" ]]; then
  echo "A4 manifest is unavailable: $manifest" >&2
  exit 3
fi
...
if [[ "$available_kib" -lt "$minimum_kib" ]]; then
  echo "Refusing A4 run: less than 20 GiB free on project filesystem" >&2
  exit 5
fi
