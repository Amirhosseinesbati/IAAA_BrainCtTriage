...
OUTPUT_DIR="reports/ich_experiments/2p5d_segmentation/diagnostics/exp70_population_trust_region_residual_train_probe_v1"
...
PYTHONPATH=. uv run python scripts/diagnose_ich_conditional_subtype_residual.py \
  --checkpoint "$CHECKPOINT" \
  --manifest "$MANIFEST" \
  --output "$OUTPUT_DIR/probe.json" \
  --run-name "ich-exp70-population-trust-region-residual-train-probe-v1" \
  --outer-fold 2 \
  --calibration-fold 1 \
  --batch-size 16 \
  --probe-batch-size 16 \
