#!/usr/bin/env bash
# Strict one-checkpoint CUDA resource screen for pre-registered MLS A5.
set -Eeuo pipefail
...
run_name="mls-vast-da-a5-detached-rank-fold0-seed42"
training_artifact_root="$campaign_root/$run_name"
audit_artifact_root="$campaign_root/a5_fold0_seed42_cuda_audit"
...
decision="$report_root/A5_FOLD0_SEED42_RESOURCE_SCREEN_DECISION.json"
...
if [[ -e "$decision" || -e "$audit_status" ]]; then
  echo "Refusing to overwrite an existing A5 audit or decision" >&2
  exit 1
fi
...
if [[ ! -f "$training_status" ]] || ! jq -e \
  ...
  echo "A5 training has not completed cleanly; refusing overlapping resource screen" >&2
  write_status "refused_training_not_completed" 3
  exit 3
fi
if [[ -f "$primary_training_report" && -f "$canonical_training_report" ]]; then
  echo "A5 training report is ambiguous across source roots" >&2
...
if [[ ! "$mlflow_run_id" =~ ^[0-9a-f]{32}$ ]]; then
  echo "Could not recover a valid MLflow run id from the completed A5 report" >&2
...
set +e
"$project_root/.venv/bin/python" scripts/evaluate_mls_a5_fold0_resource_screen.py \
...
printf 'A5 fold-0 seed-42 resource screen completed: %s\n' "$decision"
