#!/usr/bin/env bash
# One-GPU, checksum-bound launcher for the preregistered MLS A5 resource screen.
set -Eeuo pipefail
...
manifest="$project_root/config/experiments/mls-vast-deploy-aligned-a5-detached-rank-template.yaml"
...
if [[ -e "$status_path" ]]; then
  echo "Refusing to overwrite existing A5 terminal or running status: $status_path" >&2
  exit 1
fi
...
if [[ ! -f "$manifest" ]]; then
  echo "A5 manifest is unavailable: $manifest" >&2
  exit 3
fi
...
if [[ "$available_kib" -lt "$minimum_kib" ]]; then
  echo "Refusing A5 run: less than 20 GiB free on project filesystem" >&2
  exit 5
fi
