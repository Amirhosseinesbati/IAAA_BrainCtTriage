...
# Freeze the exact bytes used by this run.  A later git pull may normalize line
# endings or otherwise update the source manifest while artifact collection is
# still pending; training and transfer provenance must remain immutable.
...
printf '{"status":"running","source_manifest":"%s","manifest":"%s","manifest_sha256":"%s","started_utc":"%s","compute_policy":"cuda_only_no_cpu_fallback"}\n' \
  "$manifest" "$manifest_snapshot" "$manifest_sha" "$started_utc" > "$artifact_root/launcher_status.json"
...
printf '{"status":"%s","exit_code":%d,"source_manifest":"%s","manifest":"%s","manifest_sha256":"%s","started_utc":"%s","finished_utc":"%s","compute_policy":"cuda_only_no_cpu_fallback"}\n' \
  "$status" "$exit_code" "$manifest" "$manifest_snapshot" "$manifest_sha" "$started_utc" "$finished_utc" \
