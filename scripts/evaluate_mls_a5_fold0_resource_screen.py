"""Fail-closed A5 gate using the frozen A2/A3 resource-screen contract."""
...
def evaluate(
    ...
    result.update({
        "status": (
            "passed_for_manual_a5_replication_preregistration"
            if passed
            else "rejected_stop_a5_expansion"
        ),
...
def _log_aggregate_to_mlflow(
    ...
    """Log aggregate A5 gate evidence only; private predictions never enter MLflow."""
    try:
        ...
        for key, value in result["observed"].items():
            client.log_metric(run_id, f"a5_resource_{key}", float(value))
        for key, passed in result["gate_results"].items():
            client.log_metric(run_id, f"a5_resource_gate_{key}", float(bool(passed)))
        client.log_metric(
            run_id,
            "a5_resource_all_gates_passed",
            float(not result["failed_gates"]),
        )
        client.set_tag(run_id, "a5_resource_screen_status", str(result["status"]))
        client.set_tag(
            run_id,
            "a5_resource_screen_checkpoint_sha256",
            str(result["checkpoint_sha256"]),
        )
        client.log_artifact(
            run_id,
            str(output_path.resolve()),
            "reports/mls_deploy_aligned_a5/resource_screen",
        )
