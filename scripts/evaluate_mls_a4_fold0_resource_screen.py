"""Fail-closed A4 gate using the frozen A2/A3 resource-screen contract."""
...
def evaluate(
    ...
    """Apply the unchanged fixed contract and label the resulting A4 decision."""
    ...
    result.update({
        "status": (
            ...
            if passed
            else "rejected_stop_a4_expansion"
        ),
        "screen_scope": "a4_fold0_seed42_resource_screen_only",
        "candidate": "mls-vast-deploy-aligned-a4-pair-rank",
    })
...
def _log_aggregate_to_mlflow(
    ...
    """Log aggregate A4 gate evidence only; private predictions never enter MLflow."""
    try:
        ...
        for key, value in result["observed"].items():
            client.log_metric(run_id, f"a4_resource_{key}", float(value))
        for key, passed in result["gate_results"].items():
            client.log_metric(run_id, f"a4_resource_gate_{key}", float(bool(passed)))
        client.log_metric(
            run_id,
            "a4_resource_all_gates_passed",
            float(not result["failed_gates"]),
        )
        client.set_tag(run_id, "a4_resource_screen_status", str(result["status"]))
        client.set_tag(
            run_id,
            "a4_resource_screen_checkpoint_sha256",
            str(result["checkpoint_sha256"]),
        )
        client.log_artifact(
            run_id,
            str(output_path.resolve()),
            "reports/mls_deploy_aligned_a4/resource_screen",
        )
