...
MLFLOW_RUN_ID="${MLFLOW_RUN_ID:?MLFLOW_RUN_ID must be exported for Exp20 audit}"
...
write_status() {
  ...
  local decision="$3"
  .venv/bin/python - "$STATUS_PATH" "$state" "$evaluation_exit_code" "$decision" "$MLFLOW_RUN_ID" <<'PY'
import json
import os
import sys
...
echo "[$(date -u +%FT%TZ)] Frozen Exp20 epoch21 CUDA audit starting"
...
echo "[$(date -u +%FT%TZ)] Frozen fold2 regression-only transfer gate starting"
set +e
.venv/bin/python scripts/evaluate_mls_fixed_component_transfer.py \
  ...
  --challenger-label exp20_epoch21 \
...
echo "[$(date -u +%FT%TZ)] Uploading allowlisted aggregate Exp20 artifacts to MLflow"
.venv/bin/python scripts/log_mls_analysis_artifacts.py \
  --run-id "$MLFLOW_RUN_ID" \
  --experiment-dir "$REPORT_ROOT"

if [[ "$EVALUATION_EXIT_CODE" -eq 0 ]]; then
  DECISION="Frozen third-fold component transfer passed all primary gates."
else
  DECISION="Frozen third-fold component transfer completed and failed at least one primary gate."
fi
write_status "completed" "$EVALUATION_EXIT_CODE" "$DECISION"
FINALIZED=1
trap - EXIT

echo "[$(date -u +%FT%TZ)] Exp20 primary audit completed: $DECISION"
exit "$EVALUATION_EXIT_CODE"
