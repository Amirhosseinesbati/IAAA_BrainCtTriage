#!/usr/bin/env bash
# Strict one-checkpoint CUDA resource screen for pre-registered MLS A3.
set -Eeuo pipefail
...
audit_artifact_root="$campaign_root/a3_fold0_seed42_cuda_audit"
...
decision="$report_root/A3_FOLD0_SEED42_RESOURCE_SCREEN_DECISION.json"
...
if [[ ! -f "$training_status" ]] || ! grep -q '"state": "completed"' "$training_status"; then
  echo "A3 training has not completed cleanly; refusing overlapping resource screen" >&2
...
if [[ ! "$mlflow_run_id" =~ ^[0-9a-f]{32}$ ]]; then
  echo "Could not recover a valid MLflow run id from the completed A3 report" >&2
