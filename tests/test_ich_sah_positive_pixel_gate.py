...
from scripts.evaluate_ich_sah_positive_pixel_gate import evaluate_exp66_gate
...
def _exp66_inputs() -> tuple[dict, dict, dict, dict]:
...
def test_exp66_gate_accepts_locked_safe_gain() -> None:
    result = evaluate_exp66_gate(*_exp66_inputs())

    assert result["all_gates_passed"]
    assert result["experiment"] == "exp66_sah_positive_pixel_recovery"


def test_exp66_gate_rejects_wrong_positive_pixel_weight() -> None:
    baseline_run, candidate_run, baseline_config, candidate_config = _exp66_inputs()
    candidate_config["sah_positive_pixel_loss_weight"] = 0.01
    ...
    assert not result["candidate_recipe_checks"][
