import copy
import unittest
from scripts.evaluate_mls_a7_pair import verify_training,compare_audits,PROTOCOL_SHA,EVALUATOR_SHA,REFERENCE_SHA
...
def fixtures():
    ...
    audits={}
    for arm,mae in [('control',1.2),('consistency',1.)]:
        obs={**metrics,'mae_mm':mae,'selection_objective':mae+.4}
        audits[arm]={'status':'completed','fold':0,'seed':42,'fixed_epoch':15,'studies':70,
            'checkpoint_sha256':arm,'baseline_self_test':False,'runtime_reference_sha256':REFERENCE_SHA,
    ...
    return audits,ref,{'control':'control','consistency':'consistency'}
...
class PairAuditTest(unittest.TestCase):
    def test_matched_history_and_mismatch(self):
        ...
        histories={a:copy.deepcopy(history) for a in ['control','consistency']}
        ...
        histories['consistency'][0]['input_exposure_sha256']='x'*64
        summaries['consistency']['exposure_sha256_by_epoch'][0]='x'*64
    ...
    def test_pass_is_not_release(self):
        ...
        self.assertTrue(result['consistency_replication_review_eligible'])
        self.assertFalse(result['promotion_eligible'])
        self.assertFalse(result['automatic_replication_allowed'])

    def test_identical_model_not_consistency_benefit(self):
        audits,ref,sha=fixtures()
        audits['consistency']['observed']=copy.deepcopy(audits['control']['observed'])
        self.assertFalse(compare_audits(audits,ref,sha)['consistency_replication_review_eligible'])

    def test_false_pass_rejected(self):
        audits,ref,sha=fixtures()
        audits['consistency']['observed']['f1_3mm']=.5
    ...
    def test_runtime_checkpoint_coverage_mismatch_rejected(self):
        for key,value in [('checkpoint_sha256','wrong'),('studies',69),('hardware_signature','other'),
            ...
            audits,ref,sha=fixtures();audits['consistency'][key]=value
    ...
    def test_objective_cannot_be_fabricated(self):
        audits,ref,sha=fixtures();audits['consistency']['observed']['selection_objective']=.1
        with self.assertRaises(ValueError):compare_audits(audits,ref,sha)

    def test_nonfinite_rejected(self):
        audits,ref,sha=fixtures();audits['consistency']['observed']['mae_mm']=float('nan')
        with self.assertRaises(ValueError):compare_audits(audits,ref,sha)
