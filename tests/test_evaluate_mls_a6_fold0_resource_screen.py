...
from scripts.evaluate_mls_a6_fold0_resource_screen import (
    _atomic_json,
...
class A6Fold0ResourceScreenTests(unittest.TestCase):
    def test_launchers_are_a6_specific_and_fail_closed(self) -> None:
        root = Path(__file__).resolve().parents[1]
        train = (root / "scripts" / "run_vast_mls_a6_fold0_seed42_train.sh").read_text(
            encoding="utf-8",
        )
        resource = (
            root / "scripts" / "run_vast_mls_a6_fold0_seed42_resource_screen.sh"
        ).read_text(encoding="utf-8")
        self.assertIn('run_name="mls-vast-da-a6-local-geometry-fold0-seed42"', train)
        self.assertIn('Refusing to overwrite existing A6 terminal or running status', train)
        self.assertIn('scripts/evaluate_mls_a6_fold0_resource_screen.py', resource)
        self.assertIn('.state == "completed" and .exit_code == 0', resource)
        self.assertIn('Refusing to overwrite an existing A6 audit or decision', resource)
    ...
    def _run(self, *, mae: float = 1.4) -> dict:
        with tempfile.TemporaryDirectory() as directory:
            ...
            with mock.patch(
                ...
                "scripts.evaluate_mls_a6_fold0_resource_screen._atomic_json",
                wraps=_atomic_json,
            ) as publish:
    ...
    def test_pass_is_not_promotion_or_submission(self) -> None:
        result = self._run()
        self.assertEqual(result["candidate"], "mls-vast-deploy-aligned-a6-local-geometry")
        self.assertEqual(result["screen_scope"], "a6_fold0_seed42_resource_screen_only")
        self.assertEqual(result["status"], "passed_for_manual_a6_replication_preregistration")
        self.assertFalse(result["promotion_eligible"])
        self.assertFalse(result["submission_zip_allowed"])

    def test_failure_stops_a6_expansion(self) -> None:
        result = self._run(mae=1.5)
        self.assertEqual(result["status"], "rejected_stop_a6_expansion")
    ...
    def test_mlflow_is_aggregate_only_and_namespaced(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            result = {
                ...
                "failed_gates": [], "status": "passed_for_manual_a6_replication_preregistration", "checkpoint_sha256": "a" * 64,
            }
            ...
            self.assertTrue(all(name.startswith("a6_resource_") for name in names))
