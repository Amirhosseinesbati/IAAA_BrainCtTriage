...
Reads NIfTI volumes from nnUNet_raw and produces:
...
from src.config import ICH_LABEL_NAMES, NNUNET_RAW_DIR, PROCESSED_DIR
...
def prepare_yolo_seg_data(
    ...
    """
    Convert nnU-Net NIfTI dataset to YOLO instance segmentation format.

    Parameters
    ----------
    raw_dir : Path, optional
        nnUNet_raw directory.
    ...
    """
    raw_dir = Path(raw_dir or NNUNET_RAW_DIR)
    ...
    image_paths = sorted(images_tr.glob("*_0000.nii.gz"))
    if not image_paths:
    ...
    for vol_idx, img_path in enumerate(image_paths):
        ...
        label_path = labels_tr / img_path.name.replace("_0000.nii.gz", ".nii.gz")
        mask_3d = None
        ...
        base_name = img_path.name.replace("_0000.nii.gz", "")
