"""
mls_registry.py — Lightweight strategy registry for MLS estimation strategies.

Strategies register themselves when their module is imported.
The `src/strategies/__init__.py` triggers imports of all known
MLS strategy packages so they self-register.

Parallel to StrategyRegistry (registry.py) but for MLS strategies.
"""
...
class MLSStrategyRegistry:
    """
    Central registry that maps MLS strategy `name` → `MLSStrategy` instance.

    Usage::

        # Registration (called during import):
        MLSStrategyRegistry.register(MyMLSStrategy())

        # Lookup:
        strategy = MLSStrategyRegistry.get("mls_heatmap")
        strategy.prepare_data()
        strategy.train(config)

        # Enumeration (for UI):
        for info in MLSStrategyRegistry.list_all():
            print(info["name"], info["display_name"])
    """

    _strategies: dict[str, "MLSStrategy"] = {}

    @classmethod
    def register(cls, strategy: "MLSStrategy") -> None:
        """
        ...
        if strategy.name in cls._strategies:
            ...
            print(f"⚠️  MLSStrategyRegistry: '{strategy.name}' overwriting "
                  f"{existing.__class__.__name__} → {strategy.__class__.__name__}")
        cls._strategies[strategy.name] = strategy

    @classmethod
    def get(cls, name: str) -> "MLSStrategy":
        """
        ...
        if name not in cls._strategies:
            ...
            raise KeyError(
                f"MLS strategy '{name}' not found in registry. "
...
register = MLSStrategyRegistry.register
get = MLSStrategyRegistry.get
list_all = MLSStrategyRegistry.list_all
