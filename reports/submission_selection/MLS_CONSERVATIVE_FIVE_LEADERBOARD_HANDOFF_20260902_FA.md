...
### بررسی دامنه و اطلاعیهٔ رویداد

جست‌وجوی وب، اطلاعیهٔ کانال سومین دوره iAAA را پیدا کرد که
`https://bct-contest.iaaa.ai` را به‌عنوان لینک دسترسی دادهٔ چالش سلامت معرفی
می‌کند، نه لزوماً سامانهٔ تصحیح. همان اطلاعیه می‌گوید دسترسی پلتفرم تصحیح به‌صورت
جداگانه برای سرپرست تیم‌هایی که در پرسلاین ثبت شده‌اند ایجاد می‌شود.

بازکردن مستقیم دامنه در تاریخ این snapshot پاسخ `404 Not Found` با کد
`NoSuchWebsiteConfiguration` و bucket برابر `iaaa-contest-bct` داد. بنابراین
این دامنه در وضعیت فعلی نه صفحهٔ upload قابل استفاده است و نه مجوز حدس‌زدن مسیرهای
خصوصی را می‌دهد. لینک دعوت/حساب سرپرست تیم یا پیام رسمی حاوی آدرس سامانهٔ تصحیح
برای submission لازم است.

جست‌وجوی دقیق‌تر اطلاعیهٔ رسمی، پورتال تصحیح را مشخص کرد:
`https://platform.iaaa.ai`. طبق اطلاعیه:

- نام کاربری، ایمیل ثبت‌شدهٔ مسابقه است؛
- رمز اولیه، شماره تماس ثبت‌شده بدون صفر ابتدایی و با پسوند `.0` است؛
- تنها سرپرست تیم اجازهٔ upload مدل دارد؛
- سایر اعضا فقط نتیجه و رتبه را می‌بینند؛
- هر تیم روزانه حداکثر دو upload دارد.

صفحهٔ login رسمی در `https://platform.iaaa.ai/auth/login` فعال است. بررسی
read-only نشان داد session فعلی مرورگر به هیچ competition متصل نیست و مسیر
Overview به login بازمی‌گردد. بنابراین کاربر/سرپرست تیم باید شخصاً در پورتال
وارد شود؛ credential نباید داخل گزارش، Git، ترمینال یا متن گفتگو ثبت شود. پس از
ورود، انتخاب چالش و مشاهدهٔ وضعیت quota باید قبل از هر upload تأیید شود.
