...
def load_ground_truth(csv_path: str | Path) -> Dict[str, float]:
    ...
    result = {normalize_study_id(k): float(v) for k, v in series.items()}
...
def load_predictions_from_csv(csv_path: str | Path) -> Dict[str, float]:
    ...
    for _, row in df.iterrows():
        result[normalize_study_id(row["study_id"])] = float(row["pred_MLS_mm"])
...
def run_mls_only_inference(
    study_dirs: Dict[str, Path],
    models_dir: Path,
    device: str = "cuda",
) -> Dict[str, Any]:
    """Run ONLY the MLS models over all studies (no ICH / fracture models).

    This is the recommended mode for the MLS task leaderboard:
      - It does NOT require ICH (nnU-Net) or fracture (YOLO) weights to be
        present in ``models_dir``.
      - It keeps VRAM usage low — important on low-VRAM GPUs such as the
        GTX 1660 Ti (6 GB), where loading nnU-Net + YOLO + HRNet at once
        risks an out-of-memory error.

    The MLS model is **auto-detected** by ``submission.model.load_mls_models``:
      - ``models/mls_heatmap/mls_heatmap_best.pth`` exists → heatmap
        pipeline (HRNet + DARK, backbone/channels read from the checkpoint).
      - otherwise → legacy keypoint regression.
    ...
    Returns:
        Dict with ``predictions`` (study_id → MLS_mm) and ``mls_mode``
        (``"heatmap"`` or ``"legacy"``).
    """
    _ensure_project_on_path()
    from submission.model import load_mls_models, predict_mls_only

    logger.info("Loading MLS models only (device=%s) ...", device)
    mls_models = load_mls_models(str(models_dir), device=device)
    mls_mode = mls_models.get("mls_mode", "legacy")
    logger.info(
        "MLS models loaded (mode=%s). Running inference on %d studies ...",
        mls_mode, len(study_dirs),
    )
    ...
    for study_id, study_dir in progress:
        try:
            predictions[study_id] = float(predict_mls_only(str(study_dir), mls_models))
        except Exception as exc:
            errors.append(f"{study_id}: {exc}")
        finally:
            # Free VRAM after every study — keeps a 6 GB GTX 1660 Ti healthy.
            if device == "cuda":
                import torch
    ...
    logger.info("Inference complete: %d/%d studies.", len(predictions), len(study_dirs))
    return {"predictions": predictions, "mls_mode": mls_mode}
...
Examples:
  ...
  python -m leaderboard.task_mls --run-inference                         # MLS-only inference
                                                                         #   (auto-detects mls_heatmap)
  python -m leaderboard.task_mls --run-inference --device cpu            # MLS-only on CPU
  python -m leaderboard.task_mls --run-inference --full-pipeline         # full triage pipeline (ICH+YOLO+MLS)
        """,
    )
    parser.add_argument(
        "--input-csv",
    ...
    parser.add_argument(
        "--run-inference",
        action="store_true",
        help="Run the model pipeline instead of reading a pre-computed CSV.",
    )
    parser.add_argument(
        "--full-pipeline",
        action="store_true",
        help="(with --run-inference) Load the FULL triage pipeline "
             "(ICH + fracture + MLS) instead of MLS-only models. Heavier — "
             "not recommended on a 6 GB GPU.",
    )
    parser.add_argument(
        "--models-dir",
        type=Path,
        default=default_models,
        help=f"Models directory (default: {default_models}). The MLS model is "
             f"auto-detected: mls_heatmap/mls_heatmap_best.pth → heatmap, "
             f"else mls/keypoint_best.ckpt → legacy.",
    )
    parser.add_argument(
        "--data-dir",
        type=Path,
...
def main(argv: Optional[List[str]] = None) -> Dict[str, Any]:
    ...
    if args.run_inference:
        ...
        if args.full_pipeline:
            # Full triage pipeline (ICH + fracture + MLS) — heavier path.
            preds = run_inference(study_dirs, args.models_dir, args.device, args.ich_strategy)
            print(f"  Inference mode  : FULL pipeline ({args.ich_strategy}, {args.device})")
        else:
            # MLS-only — auto-detects heatmap vs legacy, VRAM-friendly.
            mls_info = run_mls_only_inference(study_dirs, args.models_dir, args.device)
            preds = mls_info["predictions"]
            print(f"  Inference mode  : MLS-only ({args.device})")
            print(f"  MLS model       : {mls_info['mls_mode']} (auto-detected)")
    else:
